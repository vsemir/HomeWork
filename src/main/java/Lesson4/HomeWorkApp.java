...
public class HomeWorkApp {
    ...
    private static void expPlayer(){
        if (point_Player % 3 == 0) {
            System.out.println(ANSI_YELLOW + "Поздравляем! Войдя в комнату вы видете перед собой 2 пьедестала на первом меч на втором щит. Чутье подсказывает, что можно взять только один предмет. Какой вы выбирете?" + ANSI_RESET);
            int n = in.nextInt();
            if (n == 1){
                System.out.println(ANSI_YELLOW + "Подняв меч вы почувствовали прилив сил, как будто сам Арес дал вам свое благославление! Повернувшись вы не смогли увидеть другой пьедестал." + ANSI_RESET);
                dmg_Player += 3;
            }else if (n == 2){
                System.out.println(ANSI_YELLOW + "Подняв щит вы почувствовали прилив сил, как будто сама богиня Диана дала вам свое благославление! Повернувшись вы не смогли увидеть другой пьедестал." + ANSI_RESET);
                def_Player_def += 2;
                def_Player = def_Player_def;
            }else{
                System.out.println(ANSI_YELLOW + "Увы, но вы не успели сделать выбр и теперь оба пьедестала исчезли как будто их и не было." + ANSI_RESET);
            }
        }
        if (point_Player % 6 == 0){
        ...
        if (point_Player % 10 == 0){
            System.out.println(ANSI_YELLOW + "В комнате стоял стойкий запах смрада. Похоже чем глубже идти, тем сильнее противник." + ANSI_RESET);
